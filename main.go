...
func parse(args []string) {
	parser := flags.NewParser(&v3.Commands, flags.HelpFlag)
	...
	if flagErr, ok := err.(*flags.Error); ok {
		...
		case flags.ErrHelp, flags.ErrUnknownFlag, flags.ErrExpectedArgument:
			_, found := reflect.TypeOf(v3.Commands).FieldByNameFunc(
				func(fieldName string) bool {
					field, _ := reflect.TypeOf(v3.Commands).FieldByName(fieldName)
		...
		case flags.ErrCommandRequired:
			if v3.Commands.VerboseOrVersion {
